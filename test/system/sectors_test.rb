require "application_system_test_case"

class SectorsTest < ApplicationSystemTestCase
  setup do
    @sector = sectors(:one)
  end

  test "visiting the index" do
    visit sectors_url
    assert_selector "h1", text: "Sectors"
  end

  test "creating a Sector" do
    visit sectors_url
    click_on "New Sector"

    check "Admission exam" if @sector.admission_exam
    fill_in "Admission exam date", with: @sector.admission_exam_date
    fill_in "City", with: @sector.city_id
    fill_in "Description", with: @sector.description
    fill_in "Field", with: @sector.field_id
    fill_in "Name", with: @sector.name
    fill_in "Open signup", with: @sector.open_signup
    click_on "Create Sector"

    assert_text "Sector was successfully created"
    click_on "Back"
  end

  test "updating a Sector" do
    visit sectors_url
    click_on "Edit", match: :first

    check "Admission exam" if @sector.admission_exam
    fill_in "Admission exam date", with: @sector.admission_exam_date
    fill_in "City", with: @sector.city_id
    fill_in "Description", with: @sector.description
    fill_in "Field", with: @sector.field_id
    fill_in "Name", with: @sector.name
    fill_in "Open signup", with: @sector.open_signup
    click_on "Update Sector"

    assert_text "Sector was successfully updated"
    click_on "Back"
  end

  test "destroying a Sector" do
    visit sectors_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Sector was successfully destroyed"
  end
end
