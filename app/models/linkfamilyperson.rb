class Linkfamilyperson < ApplicationRecord
has_many :linkfamilyhavetickets
has_many :tickets, through: :linkfamilyhavetickets
after_initialize do
...
def self.twopersonlink
  joins("left join twopeople t on t.id = linkfamilypeople.twoperson_id")
end
...
def self.findbyid(n)
joins('left join parentlinks links on links.id = linkfamilypeople.familylink_id').joins('left join twopeople tp on tp.id = linkfamilypeople.twoperson_id').joins('left join people p1 on p1.id = tp.person_id').joins('left join people p2 on tp.otherperson_id = p2.id').select('tp.*,linkfamilypeople.*, p1.*, p2.*, linkfamilypeople.familylink_id as linkid, linkfamilypeople.id as id,links.id as linkid2, links.name_'+I18n.locale.to_s+' as linkname, p1.name as person1, p2.name as person2,linkfamilypeople.id as worktwoid, (p1.name || \' \' || p2.name || \' \' || links.name_'+I18n.locale.to_s+') as name_'+I18n.locale.to_s).where(id: n)
end
def relationship
