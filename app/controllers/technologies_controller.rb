class TechnologiesController < ApplicationController
  before_action :set_technology, only: %i[becomes show edit update destroy ]

  # GET /technologies or /technologies.json
  def becomes
    @technology.write_attribute("type",params[:mytype])
 @technology.save
      @technology = Thing.find(params[:id])
    redirect_to @technology
  end
  def index
@tech=Technology.myhardware
  ...
  private
    ...
    def set_technology
      @technology = Technology.where(id: params[:id])[0]
    end
