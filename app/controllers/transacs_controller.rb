class TransacsController < ApplicationController
  before_action :set_transac, only: %i[ show edit update destroy comporepas ]

def comporepas
end
  # GET /transacs or /transacs.json
  def index
    @transacs = Transac.affichetransactions
  end
  ...
  # GET /transacs/new
  def new
    @transac = Transac.new(user_id:current_user.id)
  end
  ...
  private
    ...
    def transac_params
      params.require(:transac).permit(:somme, :date, :motif, :user_id)
    end
end
